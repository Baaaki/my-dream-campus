...
CREATE TABLE IF NOT EXISTS course_catalog (
    ...
    course_category course_category_enum NOT NULL DEFAULT 'theoretical',
    education_level education_level_enum NOT NULL DEFAULT 'undergraduate',
    teaching_type teaching_type_enum NOT NULL DEFAULT 'on_campus',
    ...
    status course_catalog_status_enum NOT NULL DEFAULT 'active',
...
CREATE INDEX idx_catalog_faculty ON course_catalog(faculty);
...
CREATE INDEX idx_catalog_course_category ON course_catalog(course_category);
CREATE INDEX idx_catalog_class_level ON course_catalog(class_level);
CREATE INDEX idx_catalog_semester ON course_catalog(semester);
CREATE INDEX idx_catalog_education_level ON course_catalog(education_level);
CREATE INDEX idx_catalog_language ON course_catalog(language);

-- Composite indexes for common queries
CREATE INDEX idx_catalog_dept_semester ON course_catalog(department, semester);
CREATE INDEX idx_catalog_dept_class_level ON course_catalog(department, class_level);
CREATE INDEX idx_catalog_faculty_dept ON course_catalog(faculty, department);

-- GIN indexes for JSONB columns
CREATE INDEX idx_catalog_prerequisites_gin ON course_catalog USING GIN(prerequisites);
CREATE INDEX idx_catalog_weekly_topics_gin ON course_catalog USING GIN(weekly_topics);

-- +goose Down
DROP TABLE IF EXISTS course_catalog;
DROP TYPE IF EXISTS teaching_type_enum;
