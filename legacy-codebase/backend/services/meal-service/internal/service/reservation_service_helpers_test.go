...
func TestReservationService_ValidateCancelCutoff(t *testing.T) {
	...
	t.Run("well before cutoff is allowed", func(t *testing.T) {
		...
		assert.NoError(t, s.validateCancelCutoff(reservationDate, db.MealTimeEnumLunch))
	})
	...
	t.Run("one minute past cutoff is rejected", func(t *testing.T) {
		...
		err := s.validateCancelCutoff(reservationDate, db.MealTimeEnumLunch)
	...
	t.Run("dinner uses dinner start hour", func(t *testing.T) {
		...
		err := s.validateCancelCutoff(reservationDate, db.MealTimeEnumDinner)
		assert.ErrorIs(t, err, serviceErrors.ErrCancelCutoffPassed)
	})
...
func TestReservationService_ParseMealTimeEnum(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"lunch":  {db.MealTimeEnumLunch, false},
		"dinner": {db.MealTimeEnumDinner, false},
		"":       {"", true},
		"snack":  {"", true},
...
func TestReservationService_ParseMenuTypeEnum(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"normal": {db.MenuTypeEnumNormal, false},
		"vegan":  {db.MenuTypeEnumVegan, false},
		"":       {"", true},
		"halal":  {"", true},
	}
...
func TestReservationService_ParseStatusEnum(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"pending":   {db.ReservationStatusEnumPending, false},
		"confirmed": {db.ReservationStatusEnumConfirmed, false},
		"cancelled": {db.ReservationStatusEnumCancelled, false},
		"expired":   {db.ReservationStatusEnumExpired, false},
		"":          {"", true},
		"unknown":   {"", true},
	}
