...
CREATE TYPE course_catalog.semester_status AS ENUM ('planned', 'active', 'completed');

-- Semesters table
CREATE TABLE course_catalog.semesters (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name VARCHAR(50) NOT NULL UNIQUE,
    status course_catalog.semester_status NOT NULL DEFAULT 'planned',
...
CREATE INDEX idx_semesters_status ON course_catalog.semesters(status);
CREATE INDEX idx_semesters_name ON course_catalog.semesters(name);
...
CREATE TRIGGER trg_semester_status_change
    BEFORE UPDATE OF status ON course_catalog.semesters
    FOR EACH ROW
    EXECUTE FUNCTION prevent_semester_reactivation();

-- +goose Down
DROP TRIGGER IF EXISTS trg_semester_status_change ON course_catalog.semesters;
DROP FUNCTION IF EXISTS prevent_semester_reactivation();
DROP TABLE IF EXISTS course_catalog.semesters;
DROP TYPE IF EXISTS course_catalog.semester_status;
