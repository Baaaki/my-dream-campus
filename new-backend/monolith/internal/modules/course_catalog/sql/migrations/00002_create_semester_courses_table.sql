...
CREATE TYPE course_catalog.day_of_week_enum AS ENUM ('monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday');

-- Single table for all course_catalog.semesters (no partitioning)
CREATE TABLE IF NOT EXISTS course_catalog.semester_courses (
    id UUID PRIMARY KEY DEFAULT uuidv7(),
    semester VARCHAR(50) NOT NULL,            -- "2025_spring", "2025_fall", "2026_spring"
    course_code VARCHAR(50) NOT NULL REFERENCES course_catalog.course_catalog(course_code),
    credits SMALLINT NOT NULL CHECK (credits > 0 AND credits <= 30),  -- Cached from course_catalog.course_catalog (immutable snapshot for ECTS calculation)
...
CREATE INDEX idx_semester_courses_semester ON course_catalog.semester_courses(semester);
CREATE INDEX idx_semester_courses_course_code ON course_catalog.semester_courses(course_code);
CREATE INDEX idx_semester_courses_instructor ON course_catalog.semester_courses(instructor_id);
CREATE INDEX idx_semester_courses_semester_code ON course_catalog.semester_courses(semester, course_code);

-- +goose Down
DROP TABLE IF EXISTS course_catalog.semester_courses;
DROP TYPE IF EXISTS course_catalog.day_of_week_enum;
