...
CREATE TYPE course_catalog.schedule_session_type_enum AS ENUM (
    'theory',  -- Teorik ders
    'lab'      -- Uygulama/Lab dersi
);

-- Add session_type column to course_catalog.course_schedule_sessions
ALTER TABLE course_catalog.course_schedule_sessions
    ADD COLUMN session_type course_catalog.schedule_session_type_enum NOT NULL DEFAULT 'theory';
...
CREATE INDEX idx_schedule_sessions_type ON course_catalog.course_schedule_sessions(session_type);

-- +goose Down
DROP INDEX IF EXISTS idx_schedule_sessions_type;
ALTER TABLE course_catalog.course_schedule_sessions DROP COLUMN IF EXISTS session_type;
DROP TYPE IF EXISTS course_catalog.schedule_session_type_enum;
