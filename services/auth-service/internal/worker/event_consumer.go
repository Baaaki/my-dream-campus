...
func (w *EventConsumer) Start(ctx context.Context) error {
	...
	// Declare queue
	err := w.consumer.DeclareQueue("auth_events_queue")
	if err != nil {
	...
	handler := func(body []byte) error {
		...
		case "student.updated", "staff.updated":
			return w.handleUserUpdated(ctx, body)
		case "student.deactivated", "staff.deactivated":
	...
	// Start consuming
	err = w.consumer.Consume("auth_events_queue", handler)
	if err != nil {
